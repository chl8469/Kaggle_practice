...
# ---------------------------
    ...
    kmeans = KMeans(n_clusters=36, random_state=42).fit(tmp_x_train)
    tmp_x_train['cluster'] = kmeans.predict(tmp_x_train)
    tmp_x_valid['cluster'] = kmeans.predict(tmp_x_valid)
    tmp_x_test['cluster'] = kmeans.predict(tmp_x_test)
...
def make_submission(y_test_pred, val_loss):
    ...
    df_result.to_csv('./1st_submit/model_loss_{:.4f}.csv'.format(val_loss), index=False)
